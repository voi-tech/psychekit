...
for (const instrument of instruments) {
  ...
  // The card, the questionnaire heading and the report all show the name, with the code as a secondary line.
  if (!instrument.name || !instrument.code) throw new Error(`${instrument.id}: an instrument needs both a name and a code`);
  if (instrument.name === instrument.code) throw new Error(`${instrument.id}: the name must not repeat the code`);
...
const BUNDLED_ASSETS = [
  ...
  "- License: SIL Open Font License 1.1, shipped at `public/fonts/LICENSE.txt`",
  "- Source: https://github.com/vercel/geist-font", "",
  "## Lucide icons", "",
  "- Copyright (c) Lucide Icons and Contributors",
  "- License: ISC",
  "- Source: https://lucide.dev",
  "- Only the `monitor`, `sun` and `moon` glyphs are inlined, in `src/layouts/BaseLayout.astro`.", "",
].join("\n");

const notices = [
  "# Third-party notices", "",
  "This file is generated by `scripts/validate-content.ts`. Psychological instrument content is not covered by the MIT license of the application source code.", "",
  "# Bundled assets", "",
  BUNDLED_ASSETS,
  "# Instrument licenses", "",
  "Generated from `src/content/licencje/`.", "",
