...
export function scoreScale(scale: InstrumentScale, items: InstrumentItem[], optionSets: Record<string, OptionSet>, responses: Responses): ScoreResult {
  ...
  for (const itemId of scale.items) {
    ...
    const set = optionSets[item.optionSet];
  ...
  return { scaleId: scale.id, title: scale.title, score, band: findBand(score, scale.bands), min: scale.range.min, max: scale.range.max };
}
