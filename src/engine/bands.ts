...
export function validateBands(range: Range, bands: Band[]): void {
  if (bands.length === 0 || bands[0].min !== range.min || bands[bands.length - 1].max !== range.max) throw new Error("Bands must cover the complete range");
...
export function findBand(score: number, bands: Band[]): Band {
